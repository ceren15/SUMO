...
# Yerel modüller
from .offline.landmarks import LandmarkPrecomputer
...
def _compute_replan_in_process(net_path: str, landmark_path: str, start_node: str, goal_node: str, edge_stats_snapshot: dict):
	...
	from .online.router import OnlineRouter
...
def cmd_run(args) -> int:
	...
	# SUMO entegrasyonu: bağlan, ambulans için otomatik rota oluştur, replan ve öncelik ver
	if not args.dry_run:
		try:
			from .adapters import SumoAdapter
			...
			tlc = TrafficLightController(main_tl, action_model_path="models/trigger_xgb.json")
			import time
			...
			while adapter.connected and loops < max_loops:
				...
				# Yeşil öncelik bakımını her adımda çalıştır: Ambulans 50m uzaklaşana kadar sürdür
				try:
				...
				if ambulance_id:
					...
					if snapped:
						start_node = snapped
					# TL öncelik uygulaması: belirli mesafede tetikle
					if ambulance_id:
						cand_tl_id = None
						...
						# getNextTLS ile en yakın ışık ve mesafesi
						if traci is not None:
							try:
								...
								cand_tl_id = None
						# Fallback: controlledLinks ile approach_edge'ten çöz
									...
									for group in links:
										for in_lane, _out_lane, _via in group:
											if in_lane.startswith(approach_edge + "_"):
												cand_tl_id = tl_id
												dist_to_tls = 150.0  # Varsayılan mesafe
												break
										if cand_tl_id:
											break
								except Exception:
									continue
						# XGBoost ile mesafe kontrolü
						...
						if cand_tl_id and approach_edge and should_trigger:
							logger.info(f"[TL] (XGBoost) approach={approach_edge} -> tl={cand_tl_id} karar uygulanıyor (veh={ambulance_id})")
							# Direkt set_ambulance_priority çağır - hangi yönün yeşil olacağını belirlesin ve uygulasın
							try:
...
def build_arg_parser() -> argparse.ArgumentParser:
	...
	run.add_argument("--xgb-model", default=None, help="XGBoost model dosyası (.json/.ubj)")
	run.add_argument("--xgb-action-model", default=None, help="XGBoost action sınıflandırıcı modeli (none/extend/switch)")
	run.add_argument("--xgb-extend-model", default=None, help="XGBoost extend süresi regresyon modeli")
