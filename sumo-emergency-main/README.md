...
- Yaklaşılan koridora trafik ışığı önceliği (iskelet)
...
```bash
python src/main.py prep-landmarks --net config/network_with_tl.net.xml --output data/landmarks.json
```

2) Simülasyonu başlat (GUI):
```bash
python src/main.py run --config config/simulation.sumocfg --gui --replan-interval 10 --spawn-period 60
```
...
```bash
# Landmark üret
python src/main.py prep-landmarks --net config/network_with_tl.net.xml

# GUI ile çalıştır, 10 sn replan, varsayılan spawn periyodu
python src/main.py run --config config/simulation.sumocfg --gui --replan-interval 10

# Test için daha hızlı
python src/main.py run --config config/simulation.sumocfg --gui --replan-interval 6 --spawn-period 30
```
...
- `src/main.py` (cmd_run):
  ...
  - Kenar kalemleri/ETA loglama; sinyal önceliği iskeleti
